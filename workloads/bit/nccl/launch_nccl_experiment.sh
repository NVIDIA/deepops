...
export EXPNAME=${nodes_per_job}node_NCCL_$(date +%Y%m%d%H%M%S)
#if [ x"${hplai}" == x"1" ]; then
#    export EXPNAME=${EXPNAME}_ai
#fi
export EXPDIR=$(pwd)/results/nccl/${EXPNAME}
...
for V in HPL_DIR HPL_SCRIPTS_DIR EXPDIR system cruntime CONT nodes_per_job gpus_per_node niters partition slurmopts maxnodes mpiopts gresstr total_nodes; do
    echo -n "${V}: "
    if [ x"${!V}" != x"" ]; then	
        echo "${!V}"
    else
...
#        fi
        ...
        CMD+="--export ALL,CONT,GPUMEM,CRUNTIME,EXPDIR "
